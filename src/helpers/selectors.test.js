...
interviewers: {
  ...
  "3": {
    id: 3,
    name: "Raho Mohamed",
    avatar: "https://example.com/raho.png"
  },
...
test("getInterviewersForDay returns an array with a length matching the number of appointments for that day", () => {
  const mondayResult = getInterviewersForDay(state, "Monday");
...
test("getInterviewersForDay returns an array containing the correct appointment objects", () => {
  ...
  expect(result).toEqual([state.interviewers["1"], state.interviewers["3"], state.interviewers["4"]]);
});
